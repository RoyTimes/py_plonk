...
class Setup(object):
    ...
    def commit(self, values: Polynomial) -> G1Point:
        ...
        if len(monomial_values.values) > len(self.powers_of_x):
            raise Exception("power of polynomial too bit")
        return ec_lincomb( [(cord, v) for (cord, v) in zip(self.powers_of_x, monomial_values.values )] )
